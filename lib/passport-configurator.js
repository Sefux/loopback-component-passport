  ...
  switch (authType) {
    ...
    case 'openid connect':
      ...
      function(req, iss, sub, profile, jwtClaims, accessToken, refreshToken,
        params, done) {
          ...
          profile.id = profile.id || profile.oid || profile._json['unique_name'];
          if (link) {
            if (req.user) {
              self.userCredentialModel.link(
              req.user.id, provider, authScheme, profile,
    ...
    case 'saml':
      strategy = new AuthStrategy(_.defaults({
        ...
        profile.id = profile.id || profile.nameID || profile._json['unique_name'] || profile['http://schemas.microsoft.com/identity/claims/objectidentifier'];
          if (link) {
            if (req.user) {
        ...
        if (session) {
          req.logIn(user, function(err) {
            ...
            if (info && info.accessToken) {
              //PATCH: sef 12.11.19: set a redirect with access_token in query string
